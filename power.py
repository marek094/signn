...
def run_data(data, self_loops=False, undirected=True, k=10, num_hops=10, eval_every=1,
  ...
  Powers_Lap_norm_meps = power_iterate((Atilde-0.0001*torch.eye(Atilde.size(0))), torch.FloatTensor(feat_X), K=num_hops, upper_tri=upper_tri)
  Powers_Lap_norm_m1 = power_iterate((Atilde-1*torch.eye(Atilde.size(0))), torch.FloatTensor(feat_X), K=num_hops, upper_tri=upper_tri)
  Powers_Lap_norm_m2 = power_iterate((Atilde-2*torch.eye(Atilde.size(0))), torch.FloatTensor(feat_X), K=num_hops, upper_tri=upper_tri)
